...
function updateRoleOnPlayerOut() {
    ...
    if (inChooseMode) {
        ...
        if (room.getScores() == null) {
            if (streak == 0 && Math.abs(teamR.length - teamB.length) == 1) { 
...
function endGame(winner) { // lida com o final de um jogo: nenhuma função stopGame dentro
    ...
    if (winner == Team.RED) {
        streak++;
        room.sendAnnouncement("🏆 Đội RED 🔴 đã thắng! | Win Streak(s): " + streak + " 🏆", null, 0xf85c5c,"italic-bold",0);
    }
    else if (winner == Team.BLUE) {
        streak = 1;
        room.sendAnnouncement("🏆 Đội BLUE 🔵 đã thắng! | Win streak(s): " + streak + " 🏆", null, 0x5cb3f8,"italic-bold",0);
    }
    else {
        streak = 0;
