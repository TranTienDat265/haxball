	...
	function givePenalty (team) {
		room.setDiscProperties(getBallIndex(), {x: team == Team.RED ? -415 : 415, y: 110, xspeed : -70, yspeed : 10});
		return;
	}
	...
	setTimeout(() =>{
        ...
        whisper("🎖          🏆 Welcome to VHB Volleyball room 🎖🏆      🎖 ", player.id, 0xD24780, "bold", 0);
		whisper("🎖    Tham gia discord VHB Volley để biết thêm nhiều chi tiết hơn nữa nhé    🎖 ", player.id, 0xD24780, "bold", 0);
		whisper("🎖    Join Discord VHB Voley:     https://discord.gg/QAKxdsak   🎖 ", player.id, 0xCCD547, "bold", 0);
		ids[player.id] = player.name;
	announce("Nhập!teamred or !teamblue để chọn team hoặc !spec để xem",player.id);
        ids[player.id] = player.name;
    announce("Type !teamred or !teamblue to select a team or !spec to view",player.id);
		updateTeams();
		resetTeamCount()
		},1000);
	}
	function start_match(){
		if (on_match == false && has_player == true) {
				announce("The match will start in 10 seconds",null,0xdb1a64)
				on_match = true
				sleep(10000).then(() => {
					room.startGame()
	...
	room.onPlayerKicked = function (kickedPlayer, reason, ban, byPlayer) {
		...
		if(ban == 0){
			console.log(kickedPlayer.name + " [" + kickedPlayer.id + "] was kicked (Người chơi này đã bị KICK (" + reason + ")");
			return false;
			}
		if(ban == 1){
			console.log(kickedPlayer.name + " [" + kickedPlayer.id + "] was banned (Người chơi này đã bị BANNED (" + reason + ")");
	...
	room.onPlayerChat = function(player, message) {
		...
		if (message.includes('@here') || message.includes('@everyone')) {
			room.kickPlayer(player.id,'Người chơi này đã dùng từ cấm lên đã bị trừng phạt!',true)
			return
		}
		if (message.startsWith("!")) {
			...
			if (args[0] == "admin" && args.length == 2) {
				...
				room.setPlayerTeam(player.id,0)
					...
					room.getPlayerList().forEach(function(player) {
						...
						if (Red_Count < 3){
							...
							whisper('Bạn đã chọn sang đội RED! Cố gắng chơi tốt nhé', player.id);
						}
						...
						if (Blue_Count < 3){
							...
							whisper('Bạn đã chọn sang đội BLUE! Cố gắng chơi tốt nhé!', player.id);
						}
				...
				if (player.admin) {
					...
					for (i = 0 ; i < ids; i++) {
						if (name == banList[i]) {
							announce("Người chơi " + name + " đã được xóa lệnh cấm!");
							banList[i] = undefined; 
						}
					}
					}
				...
				var dem = 0;
				for (var i = 0; i < ids; i++) {
					if (banList[i] != undefined) { 
						whisper("ID: " + i + " Tên: " + banList[i],player.id); 
					}
					else {
						dem += 1;
					}
				...
				if (dem == 0) {
					whisper("Không có người chơi nào bị ban!",player.id);
				}
			}
				return false;
	}
	...
	room.onGameStart = function (byPlayer) {
		...
		on_match = true
			announce("Trận đấu đang được ghi lại và đăng tải tự động lên discord VHB: https://discord.gg/QAKxdsak");
			room.startRecording();	
	}
	...
	const activities = {}
	var AFKTimeout = 60000; //In milliseconds | 60 seconds
